...
// Command buffer for non-filesystem tools
...
export function registerTools(server: Server, wsHandler: WebSocketHandler) {
  ...
  const commandBuffer: BufferedCommand[] = [];
  let commandProcessorInterval: NodeJS.Timeout | null = null;
  ...
  function processBufferedCommands(unityJustConnected: boolean) {
    if (commandBuffer.length === 0) return;
    
    const now = Date.now();
    const remainingCommands: BufferedCommand[] = [];
      ...
      if (shouldExecute) {
        if (wsHandler.isConnected()) {
          ...
          cmd.reject(new McpError(
            ...
            'Unity Editor connection timed out. Command was buffered for 120 seconds, but Unity did not connect.'
          ));
        } else {
